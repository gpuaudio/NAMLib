...
static constexpr bool g_double_buffering {true};
...
std::unique_ptr<DSP> get_dsp(const std::filesystem::path config_filename, uint32_t nframes, uint32_t nthreads) {
    dspData temp;
    return get_dsp(config_filename, temp, nframes, nthreads);
}

std::unique_ptr<DSP> get_dsp(const std::filesystem::path config_filename, dspData& returnedConfig, uint32_t nframes, uint32_t nthreads) {
    ...
    return get_dsp(conf, nframes, nthreads);
}
...
std::unique_ptr<DSP> get_dsp(dspData& conf, uint32_t nframes, uint32_t nthreads) {
    ...
    if (architecture == "LSTM") {
        ...
        if (g_double_buffering) {
            out = std::make_unique<GPUNeuralAmpModelerLSTM<ExecutionMode::eAsync>>(nframes, nthreads, num_layers, input_size, hidden_size, weights, expectedSampleRate);
        }
        else {
            out = std::make_unique<GPUNeuralAmpModelerLSTM<ExecutionMode::eSync>>(nframes, nthreads, num_layers, input_size, hidden_size, weights, expectedSampleRate);
        }
    }
        ...
        if (g_double_buffering) {
            out = std::make_unique<GPUNeuralAmpModelerWavenet<ExecutionMode::eAsync>>(nframes, nthreads, layer_array_params, head_scale, with_head, weights, expectedSampleRate);
        }
        else {
            out = std::make_unique<GPUNeuralAmpModelerWavenet<ExecutionMode::eSync>>(nframes, nthreads, layer_array_params, head_scale, with_head, weights, expectedSampleRate);
        }
    }
    else {
