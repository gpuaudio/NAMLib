...
std::unique_ptr<DSP> get_dsp(const std::filesystem::path model_file, uint32_t nframes = 64u, uint32_t nthreads = 128u);
// Creates an instance of DSP. Also returns a dspData struct that holds the data of the model.
std::unique_ptr<DSP> get_dsp(const std::filesystem::path model_file, dspData& returnedConfig, uint32_t nframes = 64u, uint32_t nthreads = 128u);
// Instantiates a DSP object from dsp_config struct.
std::unique_ptr<DSP> get_dsp(dspData& conf, uint32_t nframes = 64u, uint32_t nthreads = 128u);
}; // namespace nam
